...
function generateTerrain(){
  ...
  for (let i =0; i < cols;i++){
    for (let j = 0; j < rows; j++){
      ...
      if (j < rows * 0.5+noise(i)*5){
        terrain[i][j].type = "air";
      }else if (j >= rows * 0.5+noise(i)*3 && j < rows *0.5 + noise(i) *3 +3){
        if(player == null){
          player = new Player((i) *w + 5, j * w  - pH, pH, pW);
        }
        terrain[i][j].type = "grass";
      }else if (j >= rows *0.5 + noise(i) *3 +3 && j < rows * 0.5 +noise(i) *3+ 8){
        terrain[i][j].type = "dirt";
      }else if(j >= rows *0.5 +noise(i) *4 + 8){
